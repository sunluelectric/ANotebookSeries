...
\chapter{Shell Basics} \label{chshellbasics}
...
\subsection{Set Shell Environment Variables}\label{chshellbasics:subsec:shellenvvar}
...
Similar with \verb|PATH|, there is a list of shell environment variables for the user to monitor and control and status of the system. Table \ref{chshellbasics:tab:shellenvironmentvars} summarizes common shell environment variables. Command \verb|echo| can be used to check the values of these variables.

\begin{table}
	\centering \caption{Commonly used shell environment variables.}\label{chshellbasics:tab:shellenvironmentvars}
...
More file operations related commands are introduced in Chapter \ref{chfilemanagement}.
...
To permanently add alias to the shell, the alias needs to be added to the bash start script, which is usually \verb|~/.bashrc| for a user. See Section \ref{chshellbasics:subsec:others} for details.

\subsection{Others}\label{chshellbasics:subsec:others}
...
Shell configuration files are loaded each time a new shell starts. User-defined permanent configurations (such as useful alias) can be put into these files so that the configurations can be implemented automatically. Some useful files are summarized in Table \ref{chshellbasics:tab:shellconfig}.

\begin{table}
	\centering \caption{Shell configuration files.}\label{chshellbasics:tab:shellconfig}
	\begin{tabularx}{\textwidth}{lX}
		\hline
...
The dollar sign \verb|$| is also used to expand the value of a variable, either environmental variable or self-defined variable, as explained previously in \ref{chshellbasics:subsec:shellenvvar}.
